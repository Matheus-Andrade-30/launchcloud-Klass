...
export class UpdateQuestaoUseCase {
  ...
  async execute(input: UpdateQuestaoInput): Promise<void> {
    ...
    if (!professor || professor.role !== 'teacher')
      throw new Error('Apenas professores podem editar questões');
    ...
    if (!prova || prova.professorId !== input.professorId)
      throw new Error('Sem permissão para editar esta questão');
