...
export class MySQLRespostaVersaoRepository implements IRespostaVersaoRepository {
  async findLastVersao(alunoId: string, provaId: string, questaoId: string): Promise<RespostaVersao | null> {
    ...
    if (!r) return null;
    return new RespostaVersao(r.id, r.aluno_id, r.prova_id, r.questao_id, r.versao_num, r.s3_key, r.char_count, r.line_count, r.delta_chars, Boolean(r.suspeito), Number(r.timestamp), r.horario);
  }
  ...
  async findSuspeitosByProvaId(provaId: string): Promise<RespostaVersao[]> {
    ...
    return (rows as any[]).map(
      (r) => new RespostaVersao(r.id, r.aluno_id, r.prova_id, r.questao_id, r.versao_num, r.s3_key, r.char_count, r.line_count, r.delta_chars, Boolean(r.suspeito), Number(r.timestamp), r.horario),
    );
  }
  ...
  async create(versao: RespostaVersao): Promise<void> {
    await pool.execute(
      ...
      [versao.id, versao.alunoId, versao.provaId, versao.questaoId, versao.versaoNum, versao.s3Key, versao.charCount, versao.lineCount, versao.deltaChars, versao.suspeito, versao.timestamp, versao.horario],
    );
  }
}
